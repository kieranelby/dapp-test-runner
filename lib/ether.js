...
Ether.prototype.registerContract = function(name, abi, bytecode) {
  ...
  this.registeredContracts[name] = {abi: abi, bytecode: bytecode};
};
...
Ether.prototype.registerSolidityContracts = function(soliditySource) {
  ...
  for (var name in contracts) {
    if (contracts.hasOwnProperty(name)) {
      ...
      this.registerContract(name, JSON.parse(contract.abi), contract.bin);
    }
  }
};
...
Ether.prototype._sweep = function(account) {
  ...
  if (txn.value.greaterThan(0)) {
    console.log('sweeping', txn.value.toString(), 'back from account', txn.from);
