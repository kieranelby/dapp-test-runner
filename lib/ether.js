...
function Ether(runner) {
  ...
  this._web3RpcUrl = 'http://localhost:8646';
  this._availableAccountsSet = {};
  this._sweepingAccountsMap = {};
  this.registeredContracts = {};
 }
 
Ether.prototype.addEtherNode = function(etherNodeSettings) {
  if (this._nodeAdded) {
    throw new Error('use multi-ether if you need multiple nodes');
  };
  if (this.initialised) {
  ...
  if (etherNodeSettings.web3RpcUrl) {
    this._web3RpcUrl = etherNodeSettings.web3RpcUrl;
  }
  if (etherNodeSettings._nodeNumber) {
    this.nodeNumber = etherNodeSettings._nodeNumber;
  }
...
Ether.prototype.initialise = function() {
  if (this.initialised) {
    throw new Error('cannot initialise twice');
  };
  this.web3.setProvider(new this.web3.providers.HttpProvider(this._web3RpcUrl));
...
Ether.prototype.getMasterAccount = function() {
...
Ether.prototype.registerSolidityContracts = function(soliditySource) {
  ...
  var rawOutput = child_process.execSync('solc.exe --optimize --combined-json abi,bin', {
...
Ether.prototype.getContractInstanceAt = function(abi, address) {
...
Ether.prototype.send = function(txnObj) {
  ...
  console.log('sending transaction ', txnObj);
...
Ether.prototype._sweep = function(account) {
  ...
  if (txn.value.greaterThan(0)) {
    this.runner._log('sweeping', txn.value.toString(), 'back from account', txn.from, ' on node ', + this.nodeNumber);
...
Ether.prototype.rejectAccountUnused = function(account) {
  // no-op
};
