...
TransactionHelper.prototype._makeReplacementFunction = function(contractInstance, functionName, originalFunction) {
  ...
  // TODO - what about call and sendTransaction methods!
  if (originalFunction.getData) {
    replacementFunction.getData = originalFunction.getData;
  } else {
    replacementFunction.getData = function() { throw(new Error('not implemented')); }
  }
  replacementFunction.call = function() { throw(new Error('not implemented')); }
  replacementFunction.sendTransaction = function() { throw(new Error('not implemented')); }
  return replacementFunction;
};

TransactionHelper.prototype.send = function(txnObj) {
  var txnHash = this._helper._ether.send(txnObj);
  this._recordTxnHash(txnHash, txnObj);
  this._helper.nextStep.needsTxnMined(txnHash);
};
...
TransactionHelper.prototype._recordTxnHash = function(txnHash, optionalTxnObj) {
  this._helper._emitter.emit('testGeneratedTransaction', this._helper._wrapper, txnHash, optionalTxnObj);
};
