...
MarkdownReporter.prototype.getMarkdownReport = function() {
  ...
  if (this.failedTestEntries.length > 0) {
    rpt += 'The first test that failed was ' + this._quotedTitleOf(this.failedTestEntries[0].test) + doubleNewline;
  }
  ...
  for (var testIdx = 0; testIdx < this.testEntries.length; testIdx++) {
    ...
    rpt += '### Test - ' + this._quotedTitleOf(test) + doubleNewline;
    ...
    if (testEntry.test.failed) {
      ...
      var line = this._extractLineFromErr(test.err);
      if (line !== undefined) {
        rpt += ' ' + line;
      }
      rpt += doubleNewline;
    ...
    if (testEntry.txnHashes.length > 0) {
      ...
      for (var txnHashIdx = 0; txnHashIdx < testEntry.txnHashes.length; txnHashIdx++) {
        ...
        var link = '[' + txnHash + ']' + '(' + 'http://testnet.etherscan.io/tx/' + txnHash + ')';
        rpt += '* ' + link + newline;
      }
      rpt += newline;
    }
...
MarkdownReporter.prototype._quotedTitleOf = function(titledObj) {
  return '"' + titledObj.title + '"';
};

MarkdownReporter.prototype._extractLineFromErr = function(err) {
  if (!err.stack) {
    return undefined;
  }
