...
var Ether = require('./multi-ether.js');
...
var BuiltInContracts = require('./built-in-contracts.js');
...
function Runner(title) {
  ...
  this._excludeExceptCategorySet = {};
  this._testTitles = {};
  this._currentTests = [];
...
Runner.prototype.setMasterAccount = function(masterAccountSettings) {
  this._assertNotRan();
  if (this._ether.initialised) {
    throw new Error('too late, already started talking to nodes');
  }
  console.log('WARNING - not implemented');
};

Runner.prototype.addEtherNode = function(etherNodeSettings) {
  this._assertNotRan();
  if (this._ether.initialised) {
    throw new Error('too late, already started talking to nodes');
  }
  this._ether.addEtherNode(etherNodeSettings);
};

// NB: abi here should be a javascript array, not a JSON string
...
Runner.prototype.excludeExceptCategory = function(category) {
  this._excludeExceptCategorySet[category] = true;
  this._excludeExceptCategorySet['__has_entries__'] = true;
};
...
Runner.prototype.run = function(runCompletedFunction) {
  ...
  (new BuiltInContracts()).register(this);
...
Runner.prototype._advance = function() {
  ...
  if (done) {
    ...
    // perhaps we shouldn't sleep for skipped tests ...
